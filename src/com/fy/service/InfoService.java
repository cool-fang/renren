...
public interface InfoService {
    ...
    //查询当前页面是否被用户投过简历
    Jobjl findyhinthisjobcontent(int ptyh_id, int info_id) throws Exception;

    //    上传简历路径
    void uploadjl(Jobjl jobjl);

    //根据id返回info对象
    Info returninfo(int info_id) throws Exception;

    //    查询公司发布的工作类型
    List findgsjoblist(String name) throws Exception;

    //    操作用户简历
    List showjobjl(Jobjl jobjl) throws Exception;
...
//    热推职位
    ...
    //show5
    List returninfolist(String name) throws Exception;
    //show5
    Integer infototgrs(Info info) throws Exception ;
